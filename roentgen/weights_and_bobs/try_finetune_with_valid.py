...
TUNED_MODEL_SAVE_PATH_INTERMEDIATE = "/workspace/persistent/code/roentgen/weights_and_bobs/my_saved_weights_intermediate"
...
class CustomChestXRayDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #prompt = f"{row['Sex']}, age group {row['Age Group']}" # OG, before view was included
        prompt = f"{row['Sex']}, age group {row['Age Group']}, view {row['Frontal/Lateral']}" # NEW, with view included
        conditions = [
...
batch_size_uno = 28 #34 #32 # 34 out of memory eventually
train_dataloader = DataLoader(train_dataset, batch_size=batch_size_uno, shuffle=True)
val_dataloader = DataLoader(val_dataset, batch_size=batch_size_uno, shuffle=False)  # No need to shuffle validation data
...
max_timesteps = 100
best_val_loss = float("inf") # init at mad massive value

for epoch in range(num_epochs):
    #pipe.train() # error
    pipe.unet.train()
    pipe.text_encoder.train()
    for batch in train_dataloader: #dataloader:
        images = batch["image"].to(device)
        prompts = batch["prompt"]
    ...
    if avg_val_loss < best_val_loss:
        ...
        # save model weights
        print("newest best val loss, saving weights")
        pipe.save_pretrained(TUNED_MODEL_SAVE_PATH_INTERMEDIATE)
